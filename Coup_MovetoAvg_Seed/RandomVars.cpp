...
int main()
{
	srand (1);
//pullincellnums
ifstream speccells ("ZeroCoupCell.txt");
if (speccells.is_open())
{
for (int i=0; i<(cellNumber*uncoupnum); i++)
{
speccells >> zerocoupcells[i];
cout << zerocoupcells[i]<< endl;
}
speccells.close();
}
...
boost::normal_distribution<> GLYCv_hub(0.000126, 0.0000126);//simulate hubs to have same top 10% mean and followers to have bottom 90% mean with small variation.
boost::normal_distribution<> GLYCv_fol(0.000126, 0.0000126); //sigma is sigma/4
...
for(int i=0;i<cellNumber;i++)
{
	if(std::find(zerocoupcells.begin(), zerocoupcells.end(), i) !=zerocoupcells.end()) {
		randomVars[i][1] = 0.12; //set to average gKATP
	} else {
		vSum = vSum+randomVars[i][1];//use the sum of the rest of the cells to readjust them to have an average of the overall average
	}
}

double cMean=cSum/cellNumber;
double vMean = vSum/(cellNumber-uncoupnum*cellNumber);
...
//randomVars[i][1]=randomVars[i][1]*0.12/cMean;
	if(std::find(zerocoupcells.begin(), zerocoupcells.end(), i) !=zerocoupcells.end()) {
		...
		randomVars[i][1] = randomVars[i][1]*.12/vMean;
	}	
}
///////////////////////////////////////////////////////////////////
